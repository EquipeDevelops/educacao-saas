...
export const conquistasPorUnidadeController = {
    findAll: async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const conquistas = await conquistasPorUnidadeService.findAll(instituicaoId, unidadeEscolarId);
    ...
    toggle: async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            await conquistasPorUnidadeService.toggle(conquistaId, ativo, unidadeEscolarId);
            return res.status(200).json({ message: "Status da conquista atualizado com sucesso." });
        } catch (error: any) {
            return res.status(400).json({ message: error.message || "Não foi possível atualizar o status da conquista." });
        }
    }
};