...
export const usuarioController = {
  ...
  updateCredentials: async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (user.papel === 'ALUNO' || user.papel === 'PROFESSOR') {
        if (user.id !== id) {
          return res.status(403).json({ message: 'Acesso não autorizado.' });
        }
      }
      ...
      const where: Prisma.UsuariosWhereInput = {};
      if (user.instituicaoId) where.instituicaoId = user.instituicaoId;
      if (user.papel === 'GESTOR' && user.unidadeEscolarId) {
        where.unidadeEscolarId = user.unidadeEscolarId;
      }

      const result = await UsuarioService.updateUserCredentials(
        id,
      ...
      console.error('[ERRO NO CONTROLLER - updateCredentials]:', error);
      if (error?.code === 'P2002') {
        return res.status(409).json({ message: 'Este email já está em uso.' });
      }
      ...
      if (
        error?.message ===
        'Usuário não encontrado ou sem permissão para atualizar.'
      ) {
      ...
      return res
        .status(500)
        .json({ message: error.message || 'Erro ao atualizar credenciais.' });
    }
  },
};
